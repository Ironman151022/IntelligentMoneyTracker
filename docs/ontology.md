...
The application records `recorded_at` and `captured_at` itself. Version 1 never extracts, infers, or asks the user for a transaction date or time.

| Entity            | Meaning                                    | Fields / values                                                                     | Example                                   |
| ----------------- | ------------------------------------------ | ----------------------------------------------------------------------------------- | ----------------------------------------- |
| User              | App user and payer                         | `id`, `name`                                                                        | You                                       |
| Transaction       | Current structured view of one money event | `id`, `amount`, `currency`, `recorded_at`, `status`, `updated_at`, `last_evidence_id` | ₹100 spent at Zomato                      |
...
| Evidence          | Immutable source input                     | `id`, `index_id`, `raw_text`, `type`, `captured_at`, `source_channel`               | “Paid ₹100 on Zomato” voice transcript    |
