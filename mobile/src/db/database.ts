/**
 * On-device SQLite — schema is a 1:1 port of backend/storage/schema.sql.
 * Uses expo-sqlite's synchronous API for simplicity.
 */
...
export function getDb(): SQLite.SQLiteDatabase {
  if (!_db) {
    _db = SQLite.openDatabaseSync('moneytracker.db');
...
CREATE INDEX IF NOT EXISTS idx_items_transaction         ON items (transaction_id);
CREATE INDEX IF NOT EXISTS idx_tc_category               ON transaction_categories (category_id);
CREATE INDEX IF NOT EXISTS idx_tb_beneficiary            ON transaction_beneficiaries (beneficiary_id);
CREATE INDEX IF NOT EXISTS idx_ma_merchant               ON merchant_aliases (merchant_id);
CREATE INDEX IF NOT EXISTS idx_evaluations_chat_id       ON evaluations (chat_id);
`;
